...
- **Where a Students' Note genuinely calls for a diagram, illustration, or drawing that students would draw or copy** (e.g. a labelled diagram, a sketch of an apparatus, a map, a life cycle, a plant/animal structure), the generator does not fabricate or insert an actual image. Instead, insert a **bolded placeholder description** at the exact point the diagram belongs, clearly telling the teacher what to source or draw (e.g. **"[Diagram: labelled cross-section of a leaf, showing upper epidermis, palisade mesophyll, spongy mesophyll, vein, and lower epidermis]"**), so the teacher can locate or draw the real image before the lesson. Never leave the need for a diagram unmentioned, and never attempt to describe the image only in plain prose without this bolded, clearly bracketed placeholder format — it must stand out visually from the rest of the note as something to be sourced separately. If a genuine image-generation capability is available and produces an accurate, curriculum-appropriate image, it may be inserted directly instead of the placeholder; otherwise the bolded description placeholder is always used.
- **The Students' Note is not restricted to headings, sub-headings, paragraphs, lists, and tables alone.** Use whichever additional real notebook conventions the content genuinely calls for — e.g. labelled diagram placeholders (as above), flowcharts or simple process chains for sequences, side notes or key-term boxes for definitions worth flagging separately, worked-example boxes, or any other layout device a real teacher would actually use on the board or in a printed note — whenever it captures the content more faithfully than a plain list, table, or paragraph would. Choose the format(s) that best fit each specific piece of content rather than defaulting to the same handful of formats regardless of what the content actually needs. must be typeset using proper mathematical formatting: true stacked fraction bars (not a plain forward slash), superscripts for powers and indices, subscripts for variable labels (e.g. v₁, u₂), root signs, degree symbols, multiplication (×) and division (÷) signs, Greek letters, and correctly aligned stepwise calculations. Use Word's native equation formatting (Insert Equation / OMML objects) for any non-trivial formula or calculation.
...
>   6. Where the concept genuinely calls for a diagram, illustration, or drawing that students would draw or copy, insert a **bolded placeholder** at that point (e.g. **"[Diagram: labelled cross-section of a leaf, showing upper epidermis, palisade mesophyll, spongy mesophyll, vein, and lower epidermis]"**) instead of an actual image, so the teacher can source or sketch it before the lesson — never omit this when a diagram is genuinely called for, and never insert a fabricated image in its place.
...
| **Diagrams**    | Reproduce diagrams from the textbook or e-note where present; where neither has one, describe them clearly and accurately, sourced from credible online or textbook references. |
...
4b. Where a Students' Note genuinely calls for a diagram, illustration, or drawing, a bolded, clearly bracketed placeholder description must be inserted at that point so the teacher can source or sketch it (never a fabricated image, and never left unmentioned).
...
10. Apply the Document Formatting Specification.
11. Confirm the Subject, Class, Term, Week, computed dates, and any mismatch or strand-merge in a brief preamble, then save and present the `.docx` file.
